...
impl Pkcs11Signer {
    ...
    fn new_with_software_fallback_policy(
        ...
        let bls_key = if load_software_fallback {
            Self::load_bls_software_fallback(&session)
        } else {
            None
        };
        let pq_key = if load_software_fallback {
            Self::load_pq_software_fallback(&session)
        } else {
            None
        };
    ...
    fn load_bls_software_fallback(session: &cryptoki::session::Session) -> Option<BlsKeypair> {
        Self::extract_data_object(session, BLS_DATA_LABEL)
            .and_then(|bytes| BlsKeypair::from_bytes(&bytes).ok())
    }

    fn load_pq_software_fallback(session: &cryptoki::session::Session) -> Option<PqKeyPair> {
        #[cfg(feature = "pq-dilithium")]
        {
            Self::extract_data_object(session, PQ_DATA_LABEL).and_then(|bytes| {
    ...
    fn create_data_object(
        ...
        // Mark BLS/PQ data objects as
        // Non-extractable and sensitive. This prevents the HSM from
        // Revealing key material via C_GetAttributeValue.
        // Note: dev/test deployments may opt into software fallback loading
        // For legacy tokens. Mainnet vendor-native startup uses
        // `new_without_software_fallback` so these secret values are never
        // Copied into process memory before hardware signing is configured.
        let template = &[
            cryptoki::object::Attribute::Class(cryptoki::object::ObjectClass::DATA),
