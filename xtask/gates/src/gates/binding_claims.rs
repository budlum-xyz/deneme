...
pub fn run(root: &Path) -> Result<String, String> {
    let f = root.join("wallet-core/src/lib.rs");
    if !f.is_file() {
...
pub fn self_test() -> Result<String, String> {
    ...
    let _ = std::fs::create_dir_all(dir.join("wallet-core/src"));
    ...
    std::fs::write(dir.join("wallet-core/src/lib.rs"), honest).map_err(|e| e.to_string())?;
    if run(&dir).is_err() {
    ...
    std::fs::write(dir.join("wallet-core/src/lib.rs"), wired).map_err(|e| e.to_string())?;
    if run(&dir).is_ok() {
        let _ = std::fs::remove_dir_all(&dir);
